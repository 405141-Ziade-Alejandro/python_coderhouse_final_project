...
class WorkStationDelete(DeleteView):
    model = WorkStation
    success_url = reverse_lazy('workstation_list')
...
def list_supplies(request):
    ...
    context = {'supplylist': supplies_query}

    return render(request, "inventory/supplies_list.html", context)

def supply_detail(request, supply_name):
    try:
        supply = Supply.objects.get(name=supply_name)
